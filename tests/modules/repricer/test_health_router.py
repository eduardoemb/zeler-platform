...
@pytest.mark.asyncio
async def test_repricer_health_registers_mongo_rabbitmq_and_registry_checks() -> None:
    app = build_app(
    ...
    body = response.json()
    assert body["module_id"] == "repricer"
    assert body["ready"] is True
    assert set(body["checks"]) == {"mongo", "rabbitmq", "registry"}


@pytest.mark.asyncio
async def test_repricer_rabbitmq_check_fails_closed_when_unreachable() -> None:
    app = build_app(
        mongo_db=FakeDb(),
...
@pytest.mark.asyncio
async def test_repricer_registry_check_fails_without_registration() -> None:
    app = build_app(
        mongo_db=FakeDb(), rabbitmq_url=TEST_RABBITMQ_URL, rabbitmq_connect=_connect_ok()
    )
