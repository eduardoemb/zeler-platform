...
@pytest.mark.asyncio
async def test_pilot_publicador_preflight_and_smoke_are_idempotent() -> None:
    stack = FakePilotStack(module="publicador", history_collection="publicador_history")
    ...
    assert first == {"module": "publicador", "history_count": 1, "response": {"status": "ok"}}
    assert second == first
