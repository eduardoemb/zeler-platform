...
@pytest.mark.asyncio
async def test_pilot_repricer_preflight_and_smoke_are_idempotent() -> None:
    stack = FakePilotStack(module="repricer", history_collection="repricer_history")
    ...
    assert first == {"module": "repricer", "history_count": 1, "response": {"status": "ok"}}
    assert second == first
