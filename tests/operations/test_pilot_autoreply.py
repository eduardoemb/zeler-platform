...
@pytest.mark.asyncio
async def test_pilot_autoreply_preflight_and_smoke_are_idempotent() -> None:
    stack = FakePilotStack(module="autoreply", history_collection="autoreply_templates")
    ...
    assert first == {"module": "autoreply", "history_count": 1, "response": {"status": "ok"}}
    assert second == first
