...
@pytest.mark.asyncio
async def test_sheets_registration_converges_seed_startup_and_restart_to_exact_13_6() -> None:
    ...
    assert len(manifest.allowed_meli_scopes) == 13
    assert "GET /item/*/performance" in manifest.allowed_meli_scopes
    assert "GET /user-product/*/performance" in manifest.allowed_meli_scopes
    ...
    assert len(first_start["allowed_meli_scopes"]) == 13
