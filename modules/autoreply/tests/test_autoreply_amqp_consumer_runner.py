...
@pytest.mark.asyncio
async def test_autoreply_runner_declares_queue_with_dlx_and_binds_manifest_routing_keys(
    ...
    monkeypatch.setattr("zeler_autoreply.consumer.aio_pika.connect_robust", fake_connect)
    runner = AutoreplyAmqpConsumerRunner(
        rabbitmq_url="amqp://unit-test",
        handler=FakeHandler(),
        manifest_path="modules/autoreply/manifest.yaml",
    )
...
@pytest.mark.asyncio
async def test_autoreply_runner_acks_message_on_successful_handler_return() -> None:
    handler = FakeHandler()
...
@pytest.mark.asyncio
async def test_autoreply_runner_nacks_with_requeue_on_runtime_error() -> None:
    runner = AutoreplyAmqpConsumerRunner(
        rabbitmq_url="amqp://unit-test",
...
@pytest.mark.asyncio
async def test_autoreply_runner_nacks_poison_message_without_requeue() -> None:
    handler = FakeHandler()
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(
        _valid_payload(),
...
@pytest.mark.asyncio
async def test_autoreply_runner_nacks_without_requeue_on_decode_error() -> None:
    handler = FakeHandler()
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
...
@pytest.mark.asyncio
async def test_autoreply_runner_close_is_idempotent_when_called_twice(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    connection = FakeConnection()
    close_calls = 0
    ...
    monkeypatch.setattr("zeler_autoreply.consumer.aio_pika.connect_robust", fake_connect)
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=FakeHandler())
...
def test_autoreply_event_from_message_decodes_valid_payload_into_autoreply_event() -> None:
    message = FakeMessage(
        {
            "event_id": "evt-autoreply-1",
