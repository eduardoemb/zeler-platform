...
from zeler_autoreply.consumer import AutoreplyAmqpConsumerRunner, AutoreplyEvent
...
class FakeHandler:
    def __init__(self, *, error: Exception | None = None) -> None:
        self.error = error
        self.events: list[AutoreplyEvent] = []
...
async def test_http_404_is_nacked_without_requeue_and_logged_to_dlq(
    ...
    handler = FakeHandler(error=_http_status_error(404, path="/questions/404"))
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(_valid_payload(resource="/questions/404"))
...
async def test_http_500_is_nacked_with_requeue_and_logs_next_attempt(
    ...
    log_spy = LogSpy()
    monkeypatch.setattr(consumer, "logger", log_spy, raising=False)
    handler = FakeHandler(error=_http_status_error(500, path="/questions/500"))
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(
        _valid_payload(resource="/questions/500"),
        headers={"x-death": [{"count": 2, "queue": "zeler.autoreply.events"}]},
    )
...
async def test_retry_budget_exhausted_nacks_without_requeue_and_logs_dlq(
    ...
    message = FakeMessage(
        _valid_payload(resource="/questions/500"),
        headers={"x-death": [{"count": 5, "queue": "zeler.autoreply.events"}]},
    )
...
async def test_connect_error_is_nacked_with_requeue_and_logged(
    ...
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(_valid_payload(resource="/questions/connect"))
    ...
    assert log_spy.warning_calls == [
        (
            "worker.message.requeued",
            {
                "event_id": "evt-1",
                "seller_id": 123456789,
                "resource_path": "/questions/connect",
                "attempt": 1,
...
async def test_timeout_error_is_nacked_with_requeue_and_logged(
    ...
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(
        _valid_payload(resource="/questions/timeout"),
        headers={"x-death": [{"count": 1, "queue": "zeler.autoreply.events"}]},
    )
    ...
    assert log_spy.warning_calls == [
        (
            "worker.message.requeued",
            {
                "event_id": "evt-1",
                "seller_id": 123456789,
                "resource_path": "/questions/timeout",
                "attempt": 2,
...
async def test_runtime_error_is_nacked_without_requeue_and_logged_to_dlq(
    ...
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(_valid_payload(resource="/questions/runtime"))
    ...
    assert log_spy.error_calls == [
        (
            "worker.message.dlq",
            {
                "event_id": "evt-1",
                "seller_id": 123456789,
                "resource_path": "/questions/runtime",
                "attempts": 1,
...
async def test_unexpected_exception_is_nacked_without_requeue_and_logged_to_dlq(
    ...
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(_valid_payload(resource="/questions/boom"))
...
async def test_successful_message_ack_is_logged_with_structured_fields(
    ...
    handler = FakeHandler()
    runner = AutoreplyAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(_valid_payload(resource="/questions/ok"))
...
def _http_status_error(status_code: int, *, path: str = "/questions/123") -> httpx.HTTPStatusError:
...
def _rate_limit_error(
    *, retry_after_seconds: int, path: str = "/questions/123"
