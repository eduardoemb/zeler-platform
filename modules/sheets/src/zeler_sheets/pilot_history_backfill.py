...
def build_pilot_history_backfill(*, db: Any, recovery_queue: Any) -> Any:
    ...
    async def history_backfill(seller_id: str) -> bool:
        ...
        if not isinstance(stored_cutoff, datetime) or stored_cutoff.tzinfo is None:
            ...
            return False
        planner = HistoryPlanner(cutoff=stored_cutoff.astimezone(UTC), months=12)
        ...
        requests = {
            chunk.id: chunk_to_recovery_request(chunk, seller_id=seller_id)
            for chunk in chunks
