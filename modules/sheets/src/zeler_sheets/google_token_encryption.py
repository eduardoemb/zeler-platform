"""KMS envelope encryption for Google OAuth tokens.

Local development uses Google Application Default Credentials. Production Cloud Run should execute
as the sheets service account with Cloud KMS Encrypter/Decrypter on the google-tokens symmetric key.
...
from zeler_sheets.sheets_config import get_settings
...
def _kms_key_name() -> str:
    ...
    return (
        ...
        f"keyRings/{settings.kms_keyring}/cryptoKeys/{settings.kms_google_tokens_key}"
    )


def _cache_key(account_id: str) -> str:
    return f"google-token-dek:{account_id}"
