...
async def test_catalog_product_http_admission_reaches_worker_and_persists(
    ...
    await recovery_db.items.insert_one(
        {
            ...
            "attributes": [],
            "variations": [{"id": 1, "catalog_product_id": "MLA2", "available_quantity": 1}],
        }
    )
    ...
    pepper = uuid4().hex
    app.state.extension_token_pepper = pepper
    queue = app.state.formula_recovery_queue
    await queue.ensure_indexes()
    await queue.enqueue(ItemInventoryRecoveryRequest(seller))
    job = await queue.claim()
    assert await queue.checkpoint_inventory(job, item_ids=["MLA10"], offset=0)
    job = await queue.claim()
    assert await queue.checkpoint_inventory(job, item_ids=["MLA10"], offset=1)
    ...
    payload = {"formula": formula, "cuenta": "PILOT", "args": {"encabezados": False}}
    async with httpx.AsyncClient(
        ...
        assert first.json()["values"] == [["DATA_UNAVAILABLE"] * width] * 2
        assert calls == []
