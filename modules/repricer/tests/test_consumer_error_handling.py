...
async def test_http_404_is_nacked_without_requeue_and_logged_to_dlq(
    ...
    handler = FakeHandler(error=_http_status_error(404, path="/items/MLA404/prices"))
...
async def test_http_500_is_nacked_with_requeue_and_logs_next_attempt(
    ...
    log_spy = LogSpy()
    monkeypatch.setattr(consumer, "logger", log_spy, raising=False)
    handler = FakeHandler(error=_http_status_error(500, path="/items/MLA500/prices"))
    runner = RepricerAmqpConsumerRunner(rabbitmq_url="amqp://unit-test", handler=handler)
    message = FakeMessage(
        _valid_payload(resource="/items/MLA500"),
        headers={"x-death": [{"count": 2, "queue": "zeler.repricer.items"}]},
    )
...
async def test_retry_budget_exhausted_nacks_without_requeue_and_logs_dlq(
    ...
    message = FakeMessage(
        _valid_payload(resource="/items/MLA500"),
        headers={"x-death": [{"count": 5, "queue": "zeler.repricer.items"}]},
    )
...
async def test_timeout_error_is_nacked_with_requeue_and_logged(
    ...
    message = FakeMessage(
        ...
        headers={"x-death": [{"count": 1, "queue": "zeler.repricer.items"}]},
    )
...
def _http_status_error(
    status_code: int,
    *,
    path: str = "/items/MLA123/prices",
) -> httpx.HTTPStatusError:
    request = httpx.Request("POST", f"http://gateway:8080/proxy/meli{path}")
...
def _rate_limit_error(
    *, retry_after_seconds: int, path: str = "/items/MLA123/prices"
