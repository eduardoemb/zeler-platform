...
- [ ] 3.3 Orders worker resume/deduplication/interruption, producer-to-publisher handoff and final coverage verified with actual Mongo and gateway doubles. The pilot orders-only protocol has completed a recent 90-order interval and an older 945-order interval with matching receipts, projections, queue states and valid reconciled marker intervals. The older interval had zero persisted orders before publication, so it adds new history. Other intervals, other resources and twelve-month coverage remain pending.
