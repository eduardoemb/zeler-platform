...
- [ ] 5.2 Verify authorized progress API/UI and nonduplicative retry. Local authorized progress snapshots, queue retry coalescence, Apps Script pending rendering, and active `/sheets/sync-jobs` retry reuse—including unique-index collision handling—are verified; zeler-app UI and live Sheets evidence remain pending.
