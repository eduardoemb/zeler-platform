...
- [ ] 3.2j Shared question scan; admission, neutral cursor/manifests, bounded detail staging and gateway page normalization are verified in the question history tests. A bounded live pilot scan observed rotating cursors and 231 unique members across three pages; the adapter ends at the verified total even when a cursor remains. The detail adapter fetches up to 20 missing verified IDs per step and resumes from receipts; live source precision is handled at the millisecond boundary. One flag-gated worker now consumes a single plan-bound question job for all twelve monthly bindings. Local Mongo tests verify bounded canonical publication, prior-proof withdrawal, newer-event preservation, inventory-checked finalization and queue completion. Production activation, actual pilot detail ingestion, full twelve-month proof and native Sheets acceptance remain pending.
...
- [ ] 3.4 Modification-page admission verified with exact [watermark−24h,cutoff) bounds, durable ID/version/hash deduplication and seller failure isolation. The bounded HTTP page adapter now uses the documented modification filters and validates pagination, seller and timestamp scope; the live pilot confirmed one first-page shape. Durable multi-page traversal, cursor/watermark finalization and runtime wiring remain pending; creation-tail rereads are not this proof.
