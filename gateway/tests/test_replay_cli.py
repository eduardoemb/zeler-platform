...
@pytest.mark.asyncio
async def test_execute_sheets_replay_publishes_to_sheets_replay_exchange_only(
    tmp_path: Any,
) -> None:
    ...
    assert database.webhook_events.update_calls == [
        (
            {"_id": "price-1", "published_at": None},
            {
                "$set": {
                    "published_at": database.webhook_events.documents[0]["published_at"],
                    "classification": "items.price_updated",
                    "replay_run_id": "ops-sheets",
                }
            },
        )
    ]
